...
2. If `$ARGUMENTS` is empty, list available files in `data/sample_claims/` and ask which to use
...
```
4. Pick one claim entry from the sample file (tell the user which path you are testing)
5. Submit it:
```bash
   curl -s -X POST http://localhost:8000/api/claims/submit \
     ...
     -d '<claim JSON>'
```
6. Report: claim_id, final status, decision, settlement amount, whether HITL triggered, errors.